...
| FR-P0c | The Cockpit stack (Inbox, Activity, Goals, Org, Approvals, Budgets, Secrets, Workspaces, Plugins) is **promoted to first-class nav areas** matching Paperclip, **reusing the existing section components** (no rebuild). | P0 | `[ ]` (P0b — next slice; a section filter on `CockpitPanel` renders each section as its own area) |
