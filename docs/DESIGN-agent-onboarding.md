...
| **ONB2** · `onboarding.txt` + JSON twin — **SHIPPED (PR #246)** | Pure `services/onboarding-doc.ts` — `buildOnboardingDoc(invite, registry, posture, baseUrlCandidates)` → JSON + text (the `.txt` renders *from* the JSON object, so they cannot drift). Public `GET /api/agent-invites/:token/onboarding[.txt]`, **exposure derived from the deployment profile** (`onboardingDocAccess`: `packaged` open · `hosted` only with `MC_ENABLE_REMOTE_ONBOARDING`; opening the *doc* never opens the *join* surface). Plus `buildOnboardingPrompt()` — the ~40-line pastable operator prompt (§3.2), returned at create-time, surfaced by ONB6. Carries the ONB1 audit's **H2**: `services/log-redaction.ts` redacts credential-shaped path segments before they reach `audit_logs.path` or the request log. | ✅ Doc is a **pure function** (no I/O, injectable `now`). Every URL carries the invite token. A worked `agentDefaultsPayload` example per allowed runtime, **read from the registry** (test-locked: notes/examples/field-lists are the registry's, so no second source of truth appears). The §1.7 claim-security paragraph is present in **both** the doc and the prompt and is asserted by a test. Every closed state → **one flat 404** (no oracle). The operator's `message` cannot forge a step. **H2 test: an `mci_inv_` token never reaches `audit_logs.path`.** |
| **ONB3** · Join request + approval gate — **SHIPPED** | Idempotent `agent_join_requests` table. Public `POST …/:token/join` (registry-validated payload; secret fields → encrypted `secrets`). Owner-gated list/approve/reject, rendered in the **existing** approvals inbox. Approve → `secureRegistration()` → agent row + token mint (hash parked). Reject → void. | ✅ **No agent row and no token exist before approval** — asserted against a real DB. ✅ Approval card is machine-generated + labelled *self-declared, unverified*, and carries no secret value. ✅ The single-use consume is one **atomic conditional UPDATE** (audit H1) with a TOCTOU concurrency test. ✅ **No free-text field in the join body** (the ONB2 re-audit's carried caveat). ✅ `perIpRateLimit()` wired (M-3). **Divergence from §3.4, deliberate:** ONB3 returns **no `claimSecret`** — the whole credential lifecycle (secret mint, hash, TTL, claim) lands in ONB4, so nothing half-built exists in between. `capabilities` is an **allow-listed enum array**, not the design's free text — a free-text field would reach `audit_logs.metadata` unredacted. |
| **ONB4** · One-time claim — **SHIPPED** | The `mcc_` claim secret is minted **at join** (`services/claim.ts`), stored **hash-only** with a TTL ≤ the invite's expiry, returned to the agent **once** in the join response. Public `POST /api/agent-join-requests/:id/claim-api-key` `{claimSecret}` spends it: **constant-time** hash compare (`arturita_bindings` pattern), the single-use consume is one **atomic conditional UPDATE** (`claimed_at IS NULL` CAS, secret cleared in the same statement), then the `mca_` token is minted under a **second CAS** (`api_token_hash IS NULL`) and stored **hash-only**. `perIpRateLimit()` wired onto the claim (the fixed socket keying, ONB3 M-2); exposure follows the profile like join. `TOKEN_CLAIM_IMPLEMENTED` flips true; the landmine guard asserts the claim route exists **iff** the constant. | ✅ Fail-closed matrix driven end to end against a real DB, and — **deviation from §3.6, deliberate, per the ONB4 build brief** — the distinct 403/409/410 codes are collapsed to **ONE identical flat 404 for every failure** (unapproved · missing agent row · wrong/expired/already-claimed secret · lost race), matching the epic's no-oracle posture. **Carries the ONB3 auditor's #3**: the claim re-reads the agent and fails closed if it is absent — never mints against `status='approved'` alone (also neutralises M-1's ghost-agent shape). Happy path → the raw token **exactly once**; hash-only at rest; `operatorCanSeeClaimedKey` stays literal `false`; a concurrency test proves two simultaneous claims yield exactly one token. |
...
Shipped as pure `backend/src/services/deployment-profile.ts`: `resolveDeploymentProfile(env)` (`MC_DEPLOYMENT_PROFILE`, **safe default `hosted`** — an unset or garbage value must resolve to the *harder* posture, because mis-reading a hosted deployment as packaged would trust a loopback we do not own) and `onboardingPosture(env)`, which returns the derived posture **plus a hardening checklist that reports, per control, whether it is satisfied and why not**. The checklist is computed, never asserted, so **no env var can talk the posture into being open before the controls that make it safe exist.**

**Correction (ONB3, 2026-07-14).** ONB1 wrote that `PUBLIC_JOIN_IMPLEMENTED` "flips in the PR that lands ONB4". It flipped in **ONB3** instead, and that is the honest place for it: the controls that make an unauthenticated *join* endpoint safe are the board-approval gate, the atomic single-use consume and the per-IP rate limit — all of which ONB3 ships — and the join surface mints no credential, so it does not depend on the claim existing. The claim is tracked by its **own** constant, **`TOKEN_CLAIM_IMPLEMENTED`** — false through ONB3, **flipped to `true` by ONB4**. Collapsing the two into one boolean would have forced a choice between a doc that promises a key it cannot give and a join surface that is built but reports itself closed. Two facts, two constants, and the landmine guard now enforces *both* directions for *each* constant (the join route exists iff `PUBLIC_JOIN_IMPLEMENTED`; **the claim route exists iff `TOKEN_CLAIM_IMPLEMENTED`** — ONB4 added the true-direction assertion, so a claim route that went missing while the constant said it exists now fails too).
