...
- **Q-M1 — Graphify semantic pass (Memory-graph, Epic M): ✅ RESOLVED (local Ollama).** Operator chose **option 2 — local Ollama (`qwen2.5:14b`)**. Ran the community-detection + LLM-naming pass incrementally over the structural graph → **110 communities, 109 named concepts, named god-node hubs, regenerated `GRAPH_REPORT.md`** (now 786 nodes/747 edges), committed to `vault/graphify-out/`. Backend `parseGraphifyGraph` surfaces `community`/`communityName` (+ tests); the tab shows concepts on hover/search/meta. **$0 — no external API, no key, nothing left the machine** (only local `localhost:11434`). Detail: `docs/DECISIONS-arturita.md` → **S-M1**.
