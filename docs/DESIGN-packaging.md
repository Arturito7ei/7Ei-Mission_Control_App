...
> **Status:** Design + story plan · **H0 spike BUILT (#268) → H1 build pipeline BUILT (#269/#270 audit) → H6 loopback auth BUILT (2026-07-15)** — see §15 (H0) + §16 (H1) + **§17 (H6)**. The design below is the RESEARCH pass; H0 proved the bundling recommendation and H1 hardened it to a production-grade, reproducible **unsigned** `.app`/`.dmg` (compiled backend; sign/notarize wired-but-inert behind a one-env-flip). **H1 STOPS for an independent audit.** · **Date:** 2026-07-15 · **Owner:** operator (arturito@7ei.ai)
...
| **H6** ✅ **BUILT (2026-07-15) — STOPPED for audit** | **Packaged-profile identity & loopback auth + fail-closed boot** | Single-operator local identity replacing Clerk on `127.0.0.1` (an OS-user-bound local session, H-Q6); **fail-closed on default `SECRETS_ENC_KEY`/`RUN_TOKEN_SECRET`** (GO-LIVE §1 follow-up); posture derivation verified `packaged`. | ✅ **MET (§17):** the secured-scope auth hook is **profile-branched** — hosted keeps `clerkAuth`, packaged swaps in `loopbackAuth` (the per-install session secret as bearer); the local operator is a seeded **owner** of the local org so `requireOrgMembership`/`requireOrgRole` pass unchanged; a request without the loopback session **401s** (not an open instance); boot **refuses** (`assertSecretKeysSafe`) on a missing/default `SECRETS_ENC_KEY`/`RUN_TOKEN_SECRET`/`MC_LOOPBACK_SESSION_SECRET`; per-install keys are generated into the **macOS Keychain**; the packaged **dashboard is reachable** as the local operator; hosted Clerk path **byte-identical + green**. | **stage→audit** (auth + fail-closed secrets) — **STOPPED HERE** | H1 |
...
## 17. H6 — RESULTS (built 2026-07-15) — real loopback auth, per-install keys, fail-closed boot

H6 closes the H0/H1 **temporary auth bypass**: the packaged profile now enforces an **authenticated single local operator** instead of skipping Clerk and 401-ing (or, worse, silently running on a throwaway key). It is **additive and profile-branched** — the hosted Clerk path is byte-identical — and it satisfies the four requirements the **H1 audit (AUDIT-H1 §LOW-3)** made mandatory. **This stage STOPS for an independent audit** (auth + fail-closed secrets — §11).

### 17.1 The dual-auth model (hosted Clerk · packaged loopback)

The secured scope's identity source is now **chosen by deployment profile**, and *only the identity source changes* — the gate is untouched:

| | **hosted** (default) | **packaged** (`MC_DEPLOYMENT_PROFILE=packaged`) |
|---|---|---|
| Secured-scope `onRequest` hook | `clerkAuth` (Clerk JWT) — **unchanged** | `loopbackAuth` (per-install session secret as bearer) |
| Identity attached | `req.auth.userId` = Clerk `sub` | `req.auth.userId` = `local-operator` |
| Membership/owner gates | `requireOrgMembership` + `requireOrgRole` | **same hooks, unchanged** — operator is a seeded owner |
| Onboarding surface | profile-gated (needs `MC_ENABLE_REMOTE_ONBOARDING`) | loopback-open (unchanged, invariant #1) |
| agent-token API | its own token auth (unchanged) | its own token auth (unchanged) |

`backend/src/index.ts` computes `securedAuthHook = profile === 'packaged' ? loopbackAuth : clerkAuth` and installs it on the **same** encapsulated scope. Because both hooks attach the identical `req.auth.userId` / `req.userId` shape, `requireOrgMembership`, `requireOrgRole('owner')`, the audit + telemetry hooks, and every route handler are **profile-agnostic** — H6 swaps the identity source and keeps the gate, exactly as required. Hosted resolves to `clerkAuth` (the safe-default profile), so the hosted secured scope is unchanged.

### 17.2 The loopback identity mechanism (H-Q6: OS-user-bound local session)

- The Electron shell generates a per-install **`MC_LOOPBACK_SESSION_SECRET`** into the **macOS login Keychain** (`apps/desktop/src/keychain.cjs`, via the zero-dep `security` CLI). A login-keychain item is readable only inside the logged-in user's unlocked session, so the identity is **OS-user-bound by construction** — a second account on the same Mac cannot read it.
- The shell injects that secret **(a)** into the forked backend's env and **(b)** as the `Authorization: Bearer …` header on **every** BrowserWindow→backend request (`session.webRequest.onBeforeSendHeaders`, `apps/desktop/src/main.cjs`). The token therefore lives **only** in the trusted main process and the loopback request — **never in page JS** (no XSS-readable credential) — and only requests from that window's session carry it.
- `backend/src/middleware/loopback-auth.ts` validates the presented bearer against `MC_LOOPBACK_SESSION_SECRET` with a **constant-time** compare (no length/timing oracle) and, on match, authenticates the request **as the single `local-operator`**. Any request without the matching secret **401s** — a second local account, a browser tab that never got the injected header, a stray localhost caller. **This is a single-operator local identity, not "no auth".**
- `backend/src/services/loopback-identity.ts` idempotently seeds the one **local org** owned by `local-operator` (+ an owner `org_members` row) on packaged boot, so the membership/owner gates resolve against a real row exactly as they do for a hosted Clerk user. `GET /api/orgs` then returns that org, so the **dashboard is usable** on first boot.
- The web build is packaged-flagged (`NEXT_PUBLIC_MC_PACKAGED=1`): the dashboard renders **as the operator** (`isSignedIn` true) instead of bouncing to the Clerk landing route (the H0 `/dashboard` gap), and `getToken()` returns a harmless non-null placeholder that the shell's header injection overrides with the real secret.

### 17.3 Per-install keys + fail-closed boot (AUDIT-H1 #1/#2/#4)

- **Generated per-install, never baked** (§5.2, req #1): on first boot the shell generates **three distinct** 32-byte random keys into the Keychain — `SECRETS_ENC_KEY`, `RUN_TOKEN_SECRET` (its **own** value, not `SECRETS_ENC_KEY` reused via the agent-api fallback), and `MC_LOOPBACK_SESSION_SECRET` — reads them back every boot, and injects them into the backend. The H0/H1 throwaway default injection is **gone**.
- **Fail-closed on default/missing (req #2/#4):** `backend/src/services/secret-keys.ts` `assertSecretKeysSafe()` runs at the very top of `start()`. In **packaged** it **refuses to boot** (throw → `process.exit(1)`) if any of the three is absent or a known dev/throwaway default (`dev-7ei-mc-secrets-key`, `dev-7ei-mc-run`, `h0-spike-local-only-not-secure`, `''`), or if `RUN_TOKEN_SECRET === SECRETS_ENC_KEY`. In **hosted** it is a **no-op** (real Fly secrets) — byte-identical boot. So **no real secret is ever encrypted under a default key**, and a mis-provisioned packaged boot fails hard rather than running open.
- Keychain generation is itself **fail-closed**: a `security` error throws in the shell; it never substitutes a default, and the backend guard would refuse it anyway.

### 17.4 The four H1-audit requirements — each satisfied

1. **Real per-install random `SECRETS_ENC_KEY` + own `RUN_TOKEN_SECRET`** → generated into the Keychain, distinct, injected (§17.3). ✅
2. **Fail-closed-on-default-key** → `assertSecretKeysSafe()` refuses the packaged boot on any default/missing key (§17.3). ✅
3. **Loopback identity gates the SAME write routes Clerk gates** → the secured scope's hook is swapped, not removed; membership + owner checks run unchanged with the operator as owner; a request without the session 401s (§17.1/§17.2, proven in `loopback-auth.test.ts`). ✅
4. **Same treatment for `RUN_TOKEN_SECRET`** → covered by the guard (present, non-default, distinct) (§17.3). ✅

### 17.5 Tests + hosted proven untouched

- **New backend tests (+14 → 1277):** `secret-keys.test.ts` (fail-closed on missing/default/reused keys; hosted no-op) and `loopback-auth.test.ts` (the hook authenticates the operator; missing/wrong/same-length bearer 401s; unconfigured secret fails closed; the operator is a real owner/member; a **real in-memory-DB** secured scope passes the membership gate under the loopback bearer and 401s without it).
- **Hosted proven byte-identical:** backend **1277/1277** tests · **11/11** evals · backend + web **typecheck clean** · the hosted `web` build (**no `DESKTOP_BUILD`**) compiles and emits **no `standalone`**; the profile-branch resolves to `clerkAuth` for hosted, and `assertSecretKeysSafe` is a no-op there. All net-new code is gated on `profile === 'packaged'` / `NEXT_PUBLIC_MC_PACKAGED`.

### 17.6 Scope discipline + honest flags

- **Verification-gated (like H1's signing):** the full **built-app** loopback flow (Keychain generation under a real `.app`, the injected-header round-trip in a live BrowserWindow) was **not** exercised end-to-end in an automated run here — the same non-interactive limitation that left H1's sign/notarize run operator-gated. The **keychain helper's** generate/read/idempotency was verified live against this machine's `security` CLI; the **backend auth + fail-closed logic** is covered by the real-DB test suite; the **web packaged flag** is verified inlined into the built dashboard chunk. The Electron main wiring (`onBeforeSendHeaders`, `provisionSecrets`) is static-correct and syntax-checked, and belongs to the H6 audit to confirm on a signed/built run.
- **Keychain ACL caveat:** on an **unsigned** app the login-keychain item's access ACL is less stable than on a signed build (macOS keys the ACL to the app's designated requirement). H1's signing run (H-Q1/Q2) makes this deterministic; until then a first read may prompt. Flagged, not hidden.
- **Untouched by design:** live adapters/feature behaviour, the audit hooks (still off), `allowShell`, the agent-token API, and the hosted Clerk path — all unchanged.

**Verdict: H6 closes the packaged auth bypass with a real single-operator loopback identity, per-install Keychain keys, and a fail-closed boot guard — additive and profile-branched, hosted byte-identical and green. STOPPED for the independent H6 audit.**
