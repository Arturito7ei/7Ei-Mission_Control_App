...
#### ⚠️ Found while building: the org payload carries a live credential

`GET /api/orgs` is `db.select().from(organisations)` (`backend/src/routes/orgs.ts`) — **the whole row** — and that row has a **`telegramBotToken`** column (`backend/src/db/schema.ts`). **The phone has received this payload since MOB-1** (`ConnectScreen` lists orgs with it); MOB-6f neither introduces nor worsens it, and narrowing the projection is a **backend** change, out of scope for an `apps/mobile`-only story. **Reported as a follow-up, not silently absorbed.**
...
**Verified:** `npm test` **142/142** · `npm run typecheck` clean · `npm run export` bundles (3.63 MB) · `npm install` clean, no ERESOLVE, react/react-dom pins and **SDK 54 untouched** (no dependency added). Additive: `apps/mobile/**` + docs only — no backend, web, or desktop file touched.
