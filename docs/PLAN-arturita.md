...
| **P0** | **Surface what we already have in Paperclip's IA** (re-fold the web nav to mirror Paperclip's information architecture — a folded/grouped/collapsible sidebar in Paperclip's group order `Overview · Workspace · Operate · Delivery · Company · General`, existing surfaces re-homed, missing areas as honest "coming soon" placeholders → Epic-P gap; reuse components, don't rebuild; everything stays reachable) | `in-progress` (**P0a** nav shell + mapping: pure `lib/navModel.ts` [8 web tests] + folded/collapsible `Sidebar.tsx` + `PlaceholderView.tsx`; 14 tabs re-homed; placeholders for Routines/Pipelines/Review-Queue/Learnings→Memory/Artifacts/Adapters/Members/Search; collapse+fold persisted; `docs/IA-paperclip-mapping.md`. **P0b** re-home Cockpit sections as first-class areas = next slice) | see PR | — (`docs/IA-paperclip-mapping.md`) |
