...
| **J2** | Free-first pipeline config + per-layer resolvers (LLM/STT/TTS chains + Config panel + F1 breaker on all layers) | `todo` (design/schema in PRD §2 + DECISIONS S7) | — | S7 ✅ |
| **J3** | Self-hosted STT/TTS engines (whisper.cpp · Piper/local-Chatterbox; browser Web-Speech/SpeechSynthesis = zero-install fallbacks, already live) | `todo` | — | S7 ✅ |
