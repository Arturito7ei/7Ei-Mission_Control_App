...
| **CC2** | **Propose-and-approve `machine_exec` bridge** — adapter surfaces shell/destructive ops as A2 `machine_exec` approvals (verbatim `argv` + step-up); destructive file ops via host-daemon fail-closed `approved===true`. No free exec. | `todo` | — | — |
| **CC3** | **Secure-by-default registration for code-executors** — create path sets `trustMode:'low_trust_review'` + explicit `permissions` + `trustBoundary`; new `machine_exec` capability; closes the allow-all-when-empty footgun for this runtime. | `todo` | — | — |
| **CC4** | **UI/CLI adapter wiring** — `HireDialog`/`AddAgentWizard` get a `claude_code` adapter branch + `mc.env` profile (today `claude_code` falls through to the OpenClaw run-command); optional `externalEndpoint` field. | `todo` | — | — |
| **CC5** | **Command allowlist/denylist (unlock autonomy)** — build the missing semantic command control (no shell-command denylist exists today); integrate with the `machine_exec` `allowlisted` render. Gates the step from propose-and-approve → autonomous shell exec. | `todo` | — | — |
| **CC6** | **Docs / DX / go-live** — `adapters/claude-code/README.md`, update `adapters/CLAUDE.md` + `GO-LIVE.md` prereqs (Claude CLI on host, token, workboundary), `STATUS.md` + vault milestone. | `todo` | — | — |
