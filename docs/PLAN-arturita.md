...
| **CC1** | **Claude Code adapter (poll loop)** — `adapters/claude-code/` daemon (mirror `openclaw/mc_adapter.py`): onboard as `runtime:'claude_code'`, poll `/tasks?state=assigned`, claim, run headless `claude -p` in the workspace/`cc/`-worktree, stream `/runs/:id/log`, post `/result` + `/heartbeat`. Makes Claude Code a real fleet executor. | `todo` (design shipped — `docs/DESIGN-claude-code-agent.md`; office side already reuse: `claude_code` runtime accepted end-to-end, no backend changes for MVP) | see DESIGN doc | — (`docs/DESIGN-claude-code-agent.md`) |
