...
- **Coverage observations (not failures): 3 → now CLOSED** — the minor test-coverage gaps (MTI-03 ownerId-strip, CCS-04 migration idempotency re-run, AVL-10 Arturita-panel blank-key path) each got a dedicated regression test in a test-only follow-up. None ever affected shipped behavior; see the [Coverage observations](#coverage-observations-now-closed) section for the new test names. Backend suite is now **1280/1280** (+3).
...
| AVL-10 | Arturita | Blank key on re-save preserves stored key | P1 | Pass (auto) | `custom-model-agent.test.ts:140` "re-saving without a key keeps the stored one"; `:152` "explicit empty key clears". _Coverage note #3 **now closed:** `custom-model.test.ts` "applyCustomModel (pipeline default) treats an OMITTED key the same as empty — no keep-on-blank (the pipeline panel is add-only)" pins the pipeline panel's actual behaviour (undefined ≡ '' → clear). The keep-on-blank convention is the AGENT panel's; the pipeline panel is add-only (web `CustomModelForm`), so a blank key always means "add a keyless entry" — see the resolved note #3._ |
...
| MTI-03 | Multi-Tenant | Owner via ownerId keeps access; PATCH strips ownerId | P0 | Pass (auto) | `membership-scoping.test.ts:346` "GRANDFATHER: legacy org OWNER with no org_members row not locked out; outsider still 403s". ownerId-strip **code-verified** at `backend/src/routes/orgs.ts:149-153` (`const { ownerId:_o, id:_i, ...patch }`). _Coverage note #1 **now closed:** `membership-scoping.test.ts` "MTI-03: a MEMBER cannot self-escalate ownership via PATCH /api/orgs/:orgId (ownerId + id stripped)" drives the real route through the real gate and asserts ownerId/id unchanged while a legit field applies._ |
...
| CCS-04 | Cross-Cutting | Migrations idempotent, safe defaults, no backfill | P1 | Pass (auto) | Additive columns present + nullable defaults: `db-001.test.ts:6-26`, `sprint4.test.ts`, `sprint7.test.ts:10`, `cost-routes.test.ts:68`. Idempotent-ALTER convention is `backend/src/db/setup.ts` (per `backend/CLAUDE.md`); the full suite boots migrations on a fresh DB. _Coverage note #2 **now closed:** `migration-idempotency.test.ts` "CCS-04: setupDatabase re-run over an existing DB is idempotent: no throw, no data loss, ALTER-added columns preserved" seeds rows, re-runs setupDatabase twice, and asserts unchanged counts + preserved ALTER-added column values._ |
...
## Coverage observations (now closed)

These were minor **test-coverage** gaps where the shipped behavior was correct/code-verified but lacked a dedicated assertion. All three were closed by a **test-only** follow-up (no product code changed): each new test asserts the already-correct, code-verified behavior so it cannot silently regress. None was ever a defect.

| # | Sev | Case | Observation | Resolution (test-only) |
|---|---|---|---|---|
| 1 | P2 | MTI-03 | `ownerId`/`id` strip on `PATCH /api/orgs/:id` is code-verified (`orgs.ts:149-153`) but had no dedicated regression test asserting a caller cannot self-escalate ownership via PATCH. | ✅ **Covered** — `backend/src/tests/membership-scoping.test.ts` → *"MTI-03: a MEMBER cannot self-escalate ownership via PATCH /api/orgs/:orgId (ownerId + id stripped)"*. Drives the real route through the real membership gate as a plain member; asserts `ownerId`/`id` unchanged after a self-escalation payload, while a legitimate `mission` edit still applies (the strip is selective). |
| 2 | P2 | CCS-04 | Migration column-presence was tested; the "re-run migrations on an existing DB is idempotent / no backfill / existing rows unchanged" property was exercised only implicitly by suite bootstrap. | ✅ **Covered** — `backend/src/tests/migration-idempotency.test.ts` → *"CCS-04: setupDatabase re-run over an existing DB is idempotent: no throw, no data loss, ALTER-added columns preserved"*. Runs `setupDatabase()` on a fresh in-memory DB, seeds rows into ALTER-added columns, re-runs `setupDatabase()` twice, then asserts no throw, identical row counts, and preserved values (`organisations.mission`, `agents.trust_mode/avatar_url/primary_model`, `tasks.input_tokens`). |
| 3 | P2 | AVL-10 | The Arturita **pipeline** custom-model panel unit-tested only the explicit-empty-clears path (`custom-model.test.ts:87`); the "blank field preserves stored key" path was asserted for the **agent** model path (`custom-model-agent.test.ts:140`) but not for the pipeline panel. The original note hypothesized a shared `undefined`=keep / `''`=clear convention. | ✅ **Covered — and the open question resolved.** `backend/src/tests/custom-model.test.ts` → *"applyCustomModel (pipeline default) treats an OMITTED key the same as empty — no keep-on-blank (the pipeline panel is add-only)"*. **Clarification (not a bug):** the pipeline route (`arturita-custom-model.ts:56`) passes `apiKey` straight through, so `undefined` ≡ `''` → both **clear**. There is deliberately no keep-on-blank here: the pipeline UI (`web/app/dashboard/AssistantPipelineConfig.tsx` `CustomModelForm`) is **add-only** (resets every field on each add, never reopens an existing entry with a blank key), so a blank key always means "add a keyless entry" — there is no stored credential to protect. Keep-on-blank is the **agent** panel's behavior (`custom-models.ts:83-92`), which edits in place. So the doc's alternative — *"confirm the UI never sends `undefined` [as an in-place edit]"* — is the correct branch. |
