...
>
> **Epic A safety spine — in progress (overnight build 2026-07-08).** ✅ **A1 shipped** (#174): persona + command sessions + single-operator binding + `/panic` (`services/arturita-session.ts`, `arturita_sessions`/`arturita_bindings`/`arturita_nonces`, routes under `/api/orgs/:orgId/arturita/*`). ✅/🔄 **A2** (dangerous approval types + step-up — `services/dangerous-approvals.ts`, `decideApproval` step-up gate). **Current story: continue Epic A (A3 intent classifier), then the safe non-blocked work** (F1 fallback chain, E1 wallet read/prepare/simulate, plus pure planners for C1/B1 behind a fail-closed default). **Safety rule holds: no dangerous surface (B/C/D/E) merges before A2 is on `main`;** `machine_exec` (C3) and wallet signing (E2) are the last, most-guarded stories and stay blocked on **S3/S6/S4** until CONFIRMED. See `docs/QUESTIONS-arturita-overnight.md` for the morning decision list.
>
