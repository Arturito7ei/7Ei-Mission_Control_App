...
> **⚠️ DECISION LOCK 2026-07-08 — S1–S6 all CONFIRMED + two new requirements.** The operator confirmed the whole S-block and made a **material wallet-model change** (S4): from *read/prepare/never-sign, no key custody* → **bounded autonomous signing from a dedicated capped burner** (autonomous **< $100** per-tx, ≥ $100 → A2 approval; local encrypted keystore / delegated session key; **testnet only this wave, mainnet behind an explicit go flag**). Also confirmed: **S1** voice `local|provider` config (interim `provider` = Chatterbox TTS via NVIDIA API — key in the encrypted store, verified **not** committed to git; placeholder in `.env.example`), **S2** Telegram-only v1, **S3** whole-machine access + minimal self-protection denylist, **S5** push-to-talk default, **S6** broad exec (destructive still A2-gated). New **Epic H — Packaging & Distribution** (macOS signed/notarized bundle, TCC permission wizard, auto-update, fresh-machine bootstrap, v2 native iPhone app). Full detail: `docs/DECISIONS-arturita.md`; cascaded into PRD §2/§7.3/§7.4/§10/§11, PLAN (Epic E revised + Epic H new + §0 tracker), REQUIREMENTS (FR-38..42, NFR-25..27, wallet FRs). **This-wave build slice: B1 voice config+adapter+endpoint · F1 executor wiring · C1/C2 host daemon read/preview/undo (destructive A2-gated) · E2 wallet policy engine + burner keystore + testnet signing. No mainnet wallet signing; no irreversible action without the operator.**
