...
import {
  ...
  DEFAULT_DEPLOYMENT_PROFILE, PUBLIC_JOIN_IMPLEMENTED,
...
describe('[ONB1] onboarding posture — derived from the profile, never hardcoded', () => {
  ...
  it('hosted: an env flag ALONE cannot open it — the hardening checklist must be met', () => {
    const p = onboardingPosture({ MC_ENABLE_REMOTE_ONBOARDING: '1' })
    assert.equal(p.remoteOnboardingRequested, true)
    // ONB1 wires no public endpoint, so the surface stays closed however loudly
    // the config asks for it. This assertion flips only when ONB3/ONB4 land.
    assert.equal(PUBLIC_JOIN_IMPLEMENTED, false)
    assert.equal(p.publicJoinEnabled, false)
    assert.ok(p.closedBecause.some((r) => r.startsWith('join_surface_implemented')))
  })
  ...
  it('the hardening checklist reports each control honestly', () => {
    ...
    assert.deepEqual(keys, ['join_surface_implemented', 'human_approval_gate', 'invite_single_use_default', 'remote_onboarding_enabled'])
